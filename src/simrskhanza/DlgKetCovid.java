...
public class DlgKetCovid extends javax.swing.JDialog {
    ...
    public DlgKetCovid(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"dari tanggal", "sampai tanggal", "No.RM", "Nama Pasien", "J.K.", "Tmp.Lahir",
            "Tgl.Lahir", "Alamat", "Kamar", "", "No.Rawat", "No.Surat"};
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Pasien COVID ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("sansserif", 1, 12), new java.awt.Color(50, 50, 50))); // NOI18N
...
private void MnCetakKetCovidActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakKetCovidActionPerformed
    ...
    public void tampil() {
        ...
        String tgl = " and surat_ket_covid.tanggal between '" + Valid.SetDateToString(DTPTgl1.getDate()) + "' and '" + Valid.SetDateToString(DTPTgl2.getDate()) + "' ";
        String tgl1 = " surat_ket_covid.tanggal between '" + Valid.SetDateToString(DTPTgl1.getDate()) + "' and '" + Valid.SetDateToString(DTPTgl2.getDate()) + "' ";
        try {
            if (!TCari.getText().equals("")) {
                ps = koneksi.prepareStatement("select surat_ket_covid.tgl_awal,surat_ket_covid.tgl_ahir, "
                        ...
                        + "group by surat_ket_covid.no_rawat "
                        + "order by surat_ket_covid.tanggal");
            } else {
                ps = koneksi.prepareStatement("select surat_ket_covid.tgl_awal,surat_ket_covid.tgl_ahir, "
                        + "pasien.no_rkm_medis, pasien.nm_pasien, pasien.jk, pasien.tmp_lahir,pasien.tgl_lahir, "
                        + "concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab)as alamat, surat_ket_covid.kd_bangsal, surat_ket_covid.bangsal,surat_ket_covid.no_rawat,surat_ket_covid.no_surat "
...
//        String nama = Sequel.cariIsi("select nm_bangsal from bangsal where kd_bangsal='"+kd_kamar+"'");
        kdKamar.setText(kd_kamar);
        nmKamar.setText(Sequel.cariIsi("select nm_bangsal from bangsal inner join kamar on kamar.kd_bangsal=bangsal.kd_bangsal where kamar.kd_kamar='" + kd_kamar + "'"));
    }

    private void noSurat() {
        Valid.newAutoNomerPM("select ifnull(MAX(no_surat),0) from surat_ket_covid where YEAR(tanggal)='" + Valid.SetDateToString(DTPTgl.getDate()).substring(0, 4) + "' ",
                "", 5, TNoSurat);
    }
    ...
    private boolean simpan() {
        ...
        boolean status = Sequel.menyimpantf("surat_ket_covid", "'" + nomor + "','"
                + TNorawat.getText() + "','"
                + Valid.SetTgl(DTPTgl.getSelectedItem() + "") + "','"
                + Valid.SetTgl(DTPTgl3.getSelectedItem() + "") + "','"
                + Valid.SetTgl(DTPTgl.getSelectedItem() + "") + "','"
                + nmKamar.getText() + "','"
                + kdKamar.getText() + "'", "Simpan Covid");
    ...
    private void ubahData() {
        Valid.editTable(tabMode, "surat_ket_covid", "no_rawat", TNorawat,
                "tgl_awal='" + Valid.SetTgl(DTPTgl.getSelectedItem() + "")
                + "',tgl_ahir='" + Valid.SetTgl(DTPTgl3.getSelectedItem() + "")
                + "',tanggal='" + Valid.SetDateToString(new Date())
                + "',bangsal='" + nmKamar.getText()
                + "',kd_bangsal='" + kdKamar.getText() + "'");
    }
}
